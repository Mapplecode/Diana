        ...
        try:
            if get_len > input_length:
                for tag in soup.find_all('h2'):
                    ...
                    p_tag = ''
                    try:
                        p_tag = tag.findNext('p').contents[0]
                        if p_tag not in [None,'None','']:
                                GOT_P=True
                    except:
                        try:
                            p_tag = tag.parent.findNext('p').contents[0]
                            if p_tag not in [None,'None','']:
                                GOT_P=True
                        except:
                            print('NOT FOUND')
...
def paginate(doc,url,data ,previous_url=None,input_length=300,file_name='file.docx'):
    ...
    for link in links:
        ...
        if get_links.get('href') not in all_list:
            ...
            time.sleep(1)
            datas,status = click_url(show_url_list,data,input_length,file_name,doc)
            # if status:
            #     print(datas,end='\n')
            # else:
